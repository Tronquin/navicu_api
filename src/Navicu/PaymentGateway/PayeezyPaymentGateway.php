...
class PayeezyPaymentGateway extends BasePaymentGateway implements  PaymentGateway
{
    ...
    public function processPayments($request)
    {
        ...
        if (isset($response[0]) && isset($response[0]['success'])) {
            ...
            if (is_array($response[0]['response'])){
                $response[0]['response'] = json_encode($response[0]['response'], true);
            }
        }
    ...
    public function hmacAuthorizationToken($payload)
    {
        ...
        $data = $this->config['api_key'] . $this->nonce .  $this->timestamp . $this->config['merchant_token'] . $payload;
        $hashAlgorithm = "sha256";
        $hmac = hash_hmac ( $hashAlgorithm , $data , $this->config['api_secret'], false );    // HMAC Hash in hex
        $this->authorization = base64_encode($hmac);
        return array(
    ...
    public function gethmacAuthorizationToken(){
        return array(
            'authorization' => $this->authorization,
            'nonce' => $this->nonce,
            'timestamp' =>  $this->timestamp,
    ...
    /**
     * Genera un log de la compra realizada
     *
     * @param $response
     * @param $payload
     */
    private function generatePurchaseLog($response, $payload) {
        try {
            $fs = new Filesystem();

            global $kernel;
            $now = new \DateTime('now');
            $logFile = $kernel->getRootDir() . '/logs/Payeezy/' . $now->format('Y-m-d') . 'payeezy-' . $now->format('d-m-Y') .  '.log';

            if (FALSE === $response) {
                $content = (new \DateTime('now'))->format('d-m-Y h:i:s a') . ' Transaccion FALLIDA' . "\n";
            }
            else
            {
                $content = (new \DateTime('now'))->format('d-m-Y h:i:s a') . ' Transaccion COMPLETA' . "\n";
            }
     ...
     * Authorize Transaction
        if(!isset($resquest['firtsName'],$resquest['lastName'])) {
            if (isset($resquest['holder'])) {
                \explode()
            }
        }
     */
    ...
    {
        $status = 2; //default pre-reserva pendiente por pago
